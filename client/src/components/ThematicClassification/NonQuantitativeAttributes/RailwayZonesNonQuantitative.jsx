...
const NonQuantitative = () => {
  ...
  const generateConfusionMatrix = (data) => {
    const labels = Array.from(
      new Set(
        data.map((item) => item.actual).concat(data.map((item) => item.pred))
      )
    );
  ...
  return (
    <div>
      <h2 style={{ marginBottom: "10px" }}> Railways Zones Non-Quantitative</h2>
      ...
      <center>
        ...
        <Button onClick={fetchStationCode} style={{ marginBottom: "50px" }}>
          Start Test
        </Button>
      </center>
