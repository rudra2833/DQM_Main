...
const AbsolutePositionAcc = () => {
    ...
    const downloadData = async (entry) => {
        try {
            const response = await axios.get(`http://localhost:3001/api/absolutePositionAcc/download/${entry.id}`, {
                responseType: "blob",
            });
    ...
    return (
        <div className="AbsoluteApp">
            ...
            <div className="alert alert-primary" style={{margin: "0px 100px", marginBottom: "20px"}}>
                <b>Definition: </b> Absolute Position Accuracy refers to how accurately a location corresponds to its true position on the Earth's coorditates systems. (Latitude/Longitude) 
            </div>
                            ...
                            {entries.map((entry, index) => (
                                <tr key={entry.id}>
                                    ...
                                    <td>
                                        ...
                                        <button onClick={() => downloadData(entry)}>Download Data</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
