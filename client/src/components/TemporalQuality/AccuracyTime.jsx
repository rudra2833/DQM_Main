...
const AccuracyTime = () => {
  ...
  useEffect(() => {
    const numRowsToSelect = Math.ceil(
      filteredData.length * (parseFloat(selectedErrorOption) / 100)
    );
  ...
  useEffect(() => {
    ...
    const goodCount = notRandData.filter(
      (item) => item.originalData >= limits[0] && item.originalData <= limits[1]
    ).length;
    setGood(goodCount);
    const badCount = notRandData.filter(
      (item) => item.originalData <= limits[0] || item.originalData >= limits[1]
    ).length;
    setBad(badCount);
  }, [limits]);
  ...
  useEffect(() => {
    console.log("213");
    // setLimits(calculateConfidenceInterval(randData, confidenceLevel));
    const goodCount = randData.filter(
      (item) => item.originalData >= limits[0] && item.originalData <= limits[1]
    ).length;
    setGoodS(goodCount);
    const badCount = randData.filter(
      (item) => item.originalData <= limits[0] || item.originalData >= limits[1]
    ).length;
    setBadS(badCount);
  }, [limits]);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post(
        "http://localhost:3001/api/accuracymeasurement/getallcols",
        {
          filename: selectedFilename,
          attributes: target,
        }
  ...
  const fetchtableData = async () => {
    try {
      const response = await axios.post(
        "http://localhost:3001/api/accuracymeasurement/getlogs"
      );
      // console.log("hi")
  ...
  const calculateStandardDeviation = (numbers) => {
    ...
    const variance =
      squaredDifferences.reduce((acc, curr) => acc + curr, 0) / numbers.length;
  ...
  const calculateConfidenceInterval = (numbers, confidenceLevel) => {
    ...
    return [
      parseFloat(lowerLimit.toFixed(3)),
      parseFloat(upperLimit.toFixed(3)),
    ];
  };
  ...
  const zScoreLookup = (alpha) => {
    ...
    if (zScores.hasOwnProperty(alpha)) {
      ...
      console.warn(
        `Z-score for alpha ${alpha} not found in lookup table. Using approximation.`
      );
      return -1;
    }
  };
  ...
  const handleSave = async () => {
    const d = {
      ...
      good_percentage_s: parseFloat(
        ((goodS / (badS + goodS)) * 100).toFixed(2)
      ),
      notgood_percentage_s: parseFloat(
        ((badS / (badS + goodS)) * 100).toFixed(2)
      ),
      low_bound: limits[0],
      high_bound: limits[1],
    ...
    try {
      const response = await axios.post(
        "http://localhost:3001/api/accuracymeasurement/insertlog",
        d
      );
  ...
  return (
    <>
      <h2 style={{textAlign:"center"}}>Accuracy of Time Measurement</h2>
      <center>
        <div
          className="alert alert-primary"
          style={{ margin: "20px 100px", textAlign: "justify" }}
        >
          <b>Definition: </b> Closeness of reported time measurements to values
          accepted as true.
          <br />
          <b>Reference: </b>ISO 19157:2013(E) Annex D(D.5.1) - The data quality
          measures for the data quality element Accuracy of a Time Measurement
          are provided in Tables D.56 to D.61. (Page No. 93)
        </div>
        <input
        ...
        <Button onClick={attributeSelected} style={{ marginBottom: "50px" }}>
          start Test
        </Button>
      </center>
      {data.length !== 0 ? (
        <MainContainer>
                ...
                <tbody>
                  {data.map((item, index) => (
                    <TableBodyRow key={index}>
                      ...
                      <TableCell>
                        {item.originalData === "-" ? "Invalid" : "Valid"}
                      </TableCell>
                    </TableBodyRow>
                  ))}
                </tbody>
          ...
          <DataContainer>
            ...
            <div style={{ marginRight: "10px", display: "flex" }}>
              <Dropdown
                ...
                onChange={handleDropdownChangeAlpha}
              >
            ...
            <Lab>
              <strong>Mean:</strong> {parseFloat(mean).toFixed(2)}
              <br></br>
              <strong>Upper Limit: </strong>
              {limits[0]}
              <br></br>
              <strong> Lower Limit: </strong> {limits[1]}
              <br></br>
            </Lab>
            <TableWrapper
              style={{
                ...
                <tbody>
                  {randData.map((item, index) => (
                    <TableBodyRow key={index}>
                      ...
                      <TableCell>
                        {item.originalData >= limits[0] &&
                        item.originalData <= limits[1] ? (
                          <>Good</>
                        ) : (
                          <>NotGood</>
                        )}
          ...
          <DataContainer>
            ...
            <Lab>
              <strong>Good Accuracy: </strong>
              {parseFloat(((good / (bad + good)) * 100).toFixed(2))}%<br></br>
              <strong>Not Good Accuracy: </strong>
              {parseFloat(((bad / (bad + good)) * 100).toFixed(2))}%
            </Lab>
            <TableWrapper
              style={{
                height: "380px",
