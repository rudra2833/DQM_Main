  ...
  const sendFieldNames = async () => {
    try {
      ...
      if (selectedFilename) {
        ...
        const total = response.data.rows*response.data.cols;

        const omitted = response.data.omitted;
        const row = {
          selectedFilename: selectedFilename,
          total,
          omitted,
          omissionRate: (response.data.omissionRate).toFixed(2) + '%'
        };
        setTableData([row]);
        console.log(row);
        ...
        <center>
          ...
          {/* <h4>Ommission Rate: {omissionRate.toFixed(2)}%</h4> */}
            <DataTable 
              ...
              <Column
                field="omitted"
                header="Omitted Count"
                style={{ width: "25%", border: "1px solid black" }}
              ></Column>
              <Column
                field="omissionRate"
                header="Omittion Rate"
                style={{ width: "25%", border: "1px solid black" }}
              ></Column>
            </DataTable>
          <button
