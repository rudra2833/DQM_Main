  ...
  async SelectedCode(filename, attributes) {
    try {
      ...
      function isValidLatLongDMS(dmsLat, dmsLong) {
        ...
        function parseDMS(input, columnName) {
          ...
          // Determine if the input is latitude or longitude based on the column name
          let isLatitude = columnName.toLowerCase().includes("lat");
          let isLongitude = columnName.toLowerCase().includes("lon");

          // Ensure correct direction for latitude (N/S) and longitude (E/W)
          if (isLatitude && (direction === "E" || direction === "W")) {
            return null; // Invalid if latitude has 'E' or 'W'
          }
          if (isLongitude && (direction === "N" || direction === "S")) {
            return null; // Invalid if longitude has 'N' or 'S'
          }

          return dmsToDecimal(degrees, minutes, seconds, direction);
        }

        let latDecimal = parseDMS(dmsLat, typ1);
        let longDecimal = parseDMS(dmsLong, typ2);
