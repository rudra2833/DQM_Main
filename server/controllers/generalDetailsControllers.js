...
exports.saveGeneralDetails = async (req, res, next) => {
  try {
    ...
    console.log("hereee we got the file in backend!!!!!!!!!!!!!");

    const file = req.file;
    const fileSizeInBytes = file.size;
    const fileExtension = file.originalname.split(".").pop();

    const [acceptable, corrupted] = await Promise.all([
      checkFormatConsistency(fileSizeInBytes, fileExtension, res, file),
      checkCorruption(file)
    ]);

    if (!acceptable) return;
    if (corrupted) {
      return res.status(400).json({ message: "File is potentially corrupted" });
    }

    const result = await generalDetailsServices.handleData(file, file.originalname);
    res.status(201).json(result);

    console.log("hereee we processed the file in backedddddd!!!!!!!!!!!!!");
  } catch (err) {
    console.error(err);
    next(err);
  }
